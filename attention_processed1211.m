...
data_folder = '/MATLAB Drive/Attention_Tuning/attention_data_mat/attention_data_mat/Experiment_Data_Richard';
...
if strcmp(current_file, 'CeO_ExpCon_poor.mat')
        num_trials = 63; % Specific file with 63 trials
    else
        num_trials = 64; % Default number of trials
end
sz = [num_trials 8];
...
for i = 1:num_trials
    ...
    adjusted_degrees = zeros(size(degrees)); %
...
% Add a vertical line at max correlogram point
xline(peak_latency, '--r', 'Max Corr', 'LineWidth', 2, 'LabelVerticalAlignment', 'middle');
...
actually_filtered_corr(21:26, :) = 0;
...
[~, peak_time_idx] = max(max(actually_filtered_corr, [], 2));
% Extraction of values at this peak time index across all direction bins
...
% Adjusting initial parameters for the fitting
initial_params = [0.1, 0, 20, 0.05, 180, 15, -0.01]; 
lb = [0, -30, 5, 0, 150, 1, -0.01];  % Lower bounds
ub = [0.2, 30, 15, 0.2, 210, 20, 0.1];  % Upper bounds
